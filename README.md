...
3.  **Firm Production & Inventory:**
    *   **Abstraction:** Firms now act as producers. Their output is directly determined by the size of their workforce. Each tick, a firm produces a quantity of goods equal to its `num_workers` multiplied by a `production_per_worker` efficiency factor. This production is not free; it incurs a `raw_material_cost_per_unit` which is deducted from the firm's balance. This creates a direct link between labor, production, and cost.
    *   **Limitation:** The raw materials are still sourced from an abstract, infinite market at a fixed price. There is no supply chain or competition for raw materials. A firm's `target_inventory` is also static and does not yet adapt to long-term sales trends.
